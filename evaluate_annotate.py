...
if __name__ == "__main__":
    ...
    if True:
        ...
        with sess.as_default():
            ...
            test_dataset = get_batch_dataset(test_record_file, parser, FLAGS.batch_size, FLAGS.num_threads,
                                             FLAGS.capacity, True)
            ...
            def dev_step():
                ...
                while True:
                    try:
                        ...
                        context, response, step, loss, accuracy, y_pred, target = sess.run(
                            [model.context, model.response, global_step, model.loss, model.accuracy, model.y_pred,
                             model.target], feed_dict)
                        acc.append(accuracy)
                        losses.append(loss)
                ...
                with open('%s'%FLAGS.result_fname, 'w') as f:
                    for context, response, score1, score2, score3 in zip(context_list, response_list, pred_scores,
                        ...
                        f.writelines(str(context_str) + '\t' + str(response_str) + '\t' + str(score1) + '\t' + str(
                            score2) + '\t' + str(score3) + '\n')
